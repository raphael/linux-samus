...
int rt5677_spi_read(u32 addr, void *rxbuf, size_t len)
{
	...
	u8 *rx_buf;
	u8 buf[RT5677_SPI_BURST_LEN + RT5677_SPI_HEADER + 4];
	u8 spi_cmd;
	u8 *rx_data = rxbuf;
	...
	rx_buf = buf + RT5677_SPI_HEADER + 4;
	memset(t, 0, sizeof(t));
	t[0].tx_buf = buf;
	t[0].len = RT5677_SPI_HEADER + 4;
	...
	while (offset < len) {
		...
		case 6:
			...
			break;
		case 0:
			if (offset + RT5677_SPI_BURST_LEN <= len) {
				spi_cmd = RT5677_SPI_READ_BURST;
				end = RT5677_SPI_BURST_LEN;
				break;
			} else if (offset + 4 <= len) {
				spi_cmd = RT5677_SP./sound/soc/codecs/rt5677-spi.cI_READ_32;
				end = 4;
				break;
...
int rt5677_spi_write(u32 addr, const void *txbuf, size_t len)
{
	unsigned int i, end, offset = 0;
	int status = 0;
	u8 write_buf[RT5677_SPI_BURST_LEN + RT5677_SPI_HEADER + 1];
	u8 spi_cmd;
	...
	while (offset < len) {
		...
		if (spi_cmd == RT5677_SPI_WRITE_BURST) {
			...
			unsigned int j = end + (RT5677_SPI_HEADER - 1);
			for (i = 0; i < end; i++, j--) {
				if (offset + i < len)
		...
		write_buf[end + RT5677_SPI_HEADER] = spi_cmd;
		t.len = end + RT5677_SPI_HEADER + 1;
